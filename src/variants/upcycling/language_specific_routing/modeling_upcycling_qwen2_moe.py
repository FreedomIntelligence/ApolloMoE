...
class Qwen2MoeSparseMoeBlock(nn.Module):
    ...
    def forward(self, hidden_states: torch.Tensor,language_ids:Optional[torch.LongTensor] = None) -> torch.Tensor:
        ...
        if self.language_gate and self.training and self.top_k==1:
        ...
        if self.language_gate and self.training and self.top_k==1:
            if language_ids is None:
                raise ValueError('language_ids is not initialized')
            assert language_ids.shape==selected_experts.shape,f'{language_ids.shape},{selected_experts.shape}'
            selected_experts=language_ids.to(selected_experts.dtype)
            routing_weights=torch.full(routing_weights.shape,1,dtype=torch.float,device=hidden_states.device)
        ...
        if self.share_flag:
            ...
            #initialize the shared gate weights as 0.5
            # if self.language_gate1 and self.training and self.top_k==1:
            #     shared_expert_output=torch.full(self.shared_expert_gate(hidden_states).shape,0.5,dtype=hidden_states.dtype,device=hidden_states.device)*shared_expert_output
            # else:
...
class UpcyclingQwen2MoeForCausalLM(UpcyclingQwen2MoePreTrainedModel):
    ...
    def forward(
        ...
        aux_loss = None
        if output_router_logits and not self.language_gate:
