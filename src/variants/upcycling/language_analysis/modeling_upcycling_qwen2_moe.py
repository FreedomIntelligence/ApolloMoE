...
@dataclass
class UpcyclingMoeModelOutputWithPast(ModelOutput):
    ...
    token_routing:Optional[torch.LongTensor]=None


@dataclass
class UpcyclingMoeCausalLMOutputWithPast(ModelOutput):
...
class Qwen2MoeSparseMoeBlock(nn.Module):
    ...
    def forward(self, hidden_states: torch.Tensor,language_ids:Optional[torch.LongTensor] = None) -> Union[Tuple,torch.Tensor]:
        ...
        return (final_hidden_states, router_logits, selected_experts) if not self.training else (final_hidden_states, router_logits)
    ...
    def forward(
        ...
        if isinstance(hidden_states, tuple):
            if len(hidden_states)==3:
                hidden_states, router_logits, selected_expert= hidden_states
            else:
        ...
        return outputs if selected_expert is None else outputs,selected_expert
...
class UpcyclingQwen2MoePreTrainedModel(PreTrainedModel):
    ...
    def from_qwen(cls, pretrained_model_name_or_path, *model_args, **kwargs):
        ...
        print('share_flag',share_flag)
        print('attn_init_change',attn_init_change)
        print('language_gate',language_gate)
        ...
        for key in model_dict.keys():
            ...
            # if 'norm' in k and k in base_model_dict.keys():
            #     model_dict[k]=base_model_dict[k]
            #     init_keys.append(k)
            # #
            # if 'embed' in k and k in base_model_dict.keys():
            #     model_dict[k]=base_model_dict[k]
            #     init_keys.append(k)
            # #
            # if 'lm_head' in k and k in base_model_dict.keys():
            #     model_dict[k]=base_model_dict[k]
        # print(f'Initialized Parameters:{init_keys}')
...
class UpcyclingQwen2MoeModel(UpcyclingQwen2MoePreTrainedModel):
    ...
    def forward(
        ...
        cache_position: Optional[torch.LongTensor] = None,
    ) -> Union[Tuple, UpcyclingMoeModelOutputWithPast]:
        ...
        final_token_routing = torch.zeros((self.num_hidden_layers,input_ids.shape[1],self.topk), dtype=input_ids.dtype)
        for layer_index,decoder_layer in enumerate(self.layers):
            ...
            if selected_experts is not None:
                # print(selected_experts.shape,final_token_routing.shape)
                # print(input_ids.shape)
                final_token_routing[layer_index,:,:]=selected_experts
        ...
        return UpcyclingMoeModelOutputWithPast(
            last_hidden_state=hidden_states,
            past_key_values=next_cache,
            hidden_states=all_hidden_states,
            attentions=all_self_attns,
            router_logits=all_router_logits,
            token_routing=final_token_routing
        )
...
class UpcyclingQwen2MoeForCausalLM(UpcyclingQwen2MoePreTrainedModel):
    ...
    def forward(
        ...
        return_dict: Optional[bool] = None,
        cache_position: Optional[torch.LongTensor] = None,
    ) -> Union[Tuple, UpcyclingMoeCausalLMOutputWithPast]:
        ...
        return UpcyclingMoeCausalLMOutputWithPast(
            loss=loss,
            aux_loss=aux_loss,
            logits=logits,
            ...
            token_routing=outputs.token_routing
        )
