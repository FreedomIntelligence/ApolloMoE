...
class SFT_data(torch.utils.data.Dataset):
    def __init__(self, config, tokenizer):
        ...
        self.language_ids={'_ar':0,'_de':1,'_en':2,'_es':3,'_fr':4,'_hi':5,'_it':6,'_ja':7,'_ko':8,'_pt':9,'_ru':10,'_zh':11}
        # you need to set
    ...
    def __getitem__(self, index):
        ...
        la_id=key[-6:-3] if key[-6:-3] in self.language_ids.keys() else key[-3:]
...
def preprocess(args):
    ...
    for batch_cnt, batch in tqdm(enumerate(train_dataloader)):
        ...
        all_language_ids.append(cur_language_ids)
        all_attention_mask.append(cur_attention_mask)
        assert len(cur_input) == len(cur_label) ==len(cur_language_ids)==len(cur_attention_mask)== args.max_seq_len, f'{len(cur_input)},{len(cur_label)},{len(cur_attention_mask)},{len(cur_language_ids)}'
