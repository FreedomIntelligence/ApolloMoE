...
def extract_and_choose_answer(pattern, model_answer):
    ...
    #for arbic
    match_A= re.findall(r'[（\(]?(أ)[）\)]+|[（\(]+(أ)[）\)]?', model_answer)
    if match_A:
        option_count['A']=option_count.get('A', 0) + 1

    match_B= re.findall(r'[（\(]?(ب)[）\)]+|[（\(]+(ب)[）\)]?', model_answer)
    if match_B:
        option_count['B']=option_count.get('B', 0) + 1

    match_C= re.findall(r'[（\(]?(س|ك)[）\)]+|[（\(]+(س|ك)[）\)]?', model_answer)
    if match_C:
        option_count['C']=option_count.get('C', 0) + 1

    match_D= re.findall(r'[（\(]?(د)[）\)]+|[（\(]+(د)[）\)]?', model_answer)
    if match_D:
        option_count['D']=option_count.get('D', 0) + 1
...
def train(args):
    ...
    left_tokenizer = AutoTokenizer.from_pretrained(args.model_path, trust_remote_code=True, padding_side='left')#,pad_token='<|endoftext|>', eos_token='<|endoftext|>'
    ...
    config=model.config
    config.language_gate=args.language_gate
    ...
    if args.reinit:
        for name,module in model.named_modules():
            if '.mlp.gate' in name and isinstance(module,nn.Linear):
                module.weight.data.normal_(mean=0,std=config.initializer_range)
                if module.bias is not None:
                    module.bias.data.zero_()

    #freeze
    # #train 2324
    # if args.freeze_train2324:
    ...
    #
    # #train 23 24 1 2
    # if args.freeze_train232412:
    #     for name,params in model.named_parameters():
    #         params.requires_grad =False
    #         if 'layers.22' in name or 'layers.23' in name or 'layers.0' in name or 'layers.1' in name:
    #             params.requires_grad =True
    ...
    #when freeze parameters, set gradient_chekpointing=False
    # if args.gradient_checkpointing and not args.freeze_train2324 and not args.freeze_train232412:
    ...
    for epoch in range(start_epoch, args.n_epochs):
        ...
        for batch_cnt, batch in train_dataloader_iterator:
            ...
            output = model(input_ids=input_ids,labels=labels,attention_mask=attention_mask, return_dict=True,use_cache=False)
            loss = output.loss
...
if __name__ == '__main__':
    ...
    # parser.add_argument('--freeze_train2324',action='store_true')
    # parser.add_argument('--freeze_train232412',action='store_true')
