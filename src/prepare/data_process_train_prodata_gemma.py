...
class WeightedRandomSampler(Sampler[int]):
    def __init__(self, weights: Sequence[float], num_samples: int,
        ...
        if not isinstance(num_samples, int) or isinstance(num_samples, bool) or num_samples <= 0 or num_samples > len(
                weights):
            raise ValueError(
        ...
        self.rand_list = torch.multinomial(self.weights, self.weights.shape[0], self.replacement,
    ...
    def update_dynamic_weight(self, new_weights: Sequence[float]):
        ...
        new_samples = torch.multinomial(torch.as_tensor(available_weights), len(available_indices), self.replacement,
...
class SFT_data(torch.utils.data.Dataset):
    ...
    def preprocess(self, data):
        ...
        for ind, d in enumerate(data):
            if ind % 2 == 1:
                value_ids = self.tokenizer.encode(self.sep + self.roles[1] + d, add_special_tokens=False,
                ...
                value_ids = self.tokenizer.encode(pre_str + self.roles[0] + d, add_special_tokens=False,
                                                  max_length=self.config.max_seq_len, truncation=True)
                input_ids += value_ids
                if len(labels) > 0:
...
def preprocess(args):
    tokenizer = AutoTokenizer.from_pretrained(args.model_path, trust_remote_code=True, pad_token='<pad>',
                                              eos_token='<eos>')
    ...
    train_dataloader = DataLoader(train_dataset, batch_size=args.train_bsz_per_gpu, sampler=sampler, drop_last=False,
                                  collate_fn=train_dataset.collate_fn, num_workers=64)
    ...
    table.add_data(json.dumps(train_dataset.data_priority, ensure_ascii=False, indent=2),
    ...
    print(json.dumps(train_dataset.data_priority, ensure_ascii=False, indent=2),
          json.dumps(train_dataset.data_epoch, ensure_ascii=False, indent=2),
          json.dumps(train_dataset.get_data_info(), ensure_ascii=False, indent=2))
