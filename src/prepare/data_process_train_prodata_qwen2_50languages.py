...
class SFT_data(torch.utils.data.Dataset):
    def __init__(self, config, tokenizer):
        ...
        self.languages_38= [
            "mi", "ckb", "mg", "gn", "gl", "gd", "cs", "co", "ilo", "ln", "lg", "la",
            "fi", "uk", "bg", "fil", "ka", "af", "pl", "sw", "st", "sr", "sq", "nso",
            "sm", "sl", "sk", "da", "sd", "ig", "rw", "ca", "hy", "ht", "hr", "qu",
            "ha", "mt"
        ]
        # you need to set
...
def preprocess(args):
    ...
    for batch_cnt, batch in tqdm(enumerate(train_dataloader)):
        ...
        all_labels.append(cur_label)
        all_attention_mask.append(cur_attention_mask)
        assert len(cur_input) == len(cur_label)==len(cur_attention_mask)== args.max_seq_len, f'{len(cur_input)},{len(cur_label)},{len(cur_attention_mask)}'
    ...
    assert len(all_inputs_ids) == len(all_labels) ==len(all_attention_mask)
    print(len(all_inputs_ids))
    save_dataset = datasets.Dataset.from_dict({'input_ids': all_inputs_ids, 'labels':all_labels,'attention_mask':all_attention_mask})
