...
def generate_score(result_path, score_path):
    ...
    right_path=os.path.join(output_dir,'right.json')
    
    with open(all_path, "w", encoding="utf8") as f:
        json.dump(all, f, indent=4)     
    with open(right_path, "w", encoding="utf8") as f:
        json.dump(right, f, indent=4)   
...
def generate_response(args):
    ...
    tokenizer = AutoTokenizer.from_pretrained(model_path, trust_remote_code=True, padding_side='left',pad_token='<|end_of_text|>')
