...
def train(args):
    ...
    if args.checkpoint_path:
        model = AutoModelForCausalLM.from_pretrained(os.path.join(args.checkpoint_path, "tfmr"), trust_remote_code=True,attn_implementation="flash_attention_2",torch_dtype='auto')
    else:
        model = AutoModelForCausalLM.from_pretrained(args.model_path, trust_remote_code=True,attn_implementation="flash_attention_2",torch_dtype='auto')
    ...
    train_dataset = SFT_dataset(args, tokenizer)
    ...
    dev_dataset = TestDataset(args.dev_data_dir, tokenizer)
    ...
    generation_config = GenerationConfig.from_pretrained(args.model_path, pad_token_id=tokenizer.pad_token_id, num_return_sequences=1, max_new_tokens=256, min_new_tokens=2, do_sample=False, temperature=1.0, top_k=50, top_p=1.0)
