...
def train(args):
    ...
    if args.checkpoint_path:
        model = AutoModelForCausalLM.from_pretrained(os.path.join(args.checkpoint_path, "tfmr"), trust_remote_code=True,attn_implementation='eager')
    else:
        model = AutoModelForCausalLM.from_pretrained(args.model_path, trust_remote_code=True,attn_implementation='eager')
    ...
    if args.save_step <= 0:
        args.save_step=len(train_dataloader) // 40
